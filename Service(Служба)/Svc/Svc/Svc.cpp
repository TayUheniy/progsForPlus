...
VOID SvcInit(DWORD dwArgc, LPTSTR *lpszArgv)
{
	...
	while (WaitForSingleObject(ghSvcStopEvent, 0) != WAIT_OBJECT_0)
	{
		if (ConnectNamedPipe(Pipe, &ov)) {
			DWORD readByte = 0;
			TCHAR buffer[4096]{ 0 };
			ReadFile(Pipe, &buffer, 4096, &readByte,NULL);
			if (buffer) {
				if (remove(buffer) != 0) {
					DWORD writeByte = 0;
					WriteFile(Pipe, "Ошибка удаления файла", 4096, &writeByte, NULL);
					
				}
				else {
					DWORD writeByte = 0;
					WriteFile(Pipe, "Файл успешно удален", 4096, &writeByte, NULL);
